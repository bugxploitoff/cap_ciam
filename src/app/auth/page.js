...
export default function Home() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch('http://ec2-3-17-74-205.us-east-2.compute.amazonaws.com:5000/login', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        if (responseData.status === 'success') {
          Cookies.set('sessions', responseData.auth, { expires: 1 });
          setResponseStatus(responseData.status);
          setRedirectPath(responseData.path);
  ...
  useEffect(() => {
    if ((responseStatus === 'success' || responseStatus === 'cp') && redirectPath) {
      setTimeout(() => {
        // Redirect to the desired path
        window.location.href = redirectPath;
      }, 5000); // Redirect after 5 seconds
    }
  }, [responseStatus, redirectPath]);
  ...
  <div className="relative z-0 w-full mb-6 group">
      <input type="text" name="email" value={formData.email} onChange={handleChange} id="floating_email" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
  ...
  <div className="relative z-0 w-full mb-6 group">
      ...
      <label for="Invite code" className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Password</label>
  </div>
...
<br />
<h2
        className="mb-3 text-1xl font-semibold text-right cursor-pointer"
        onClick={toggleForm}
      >
             Register
